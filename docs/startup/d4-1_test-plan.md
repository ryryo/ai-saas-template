...
| ID | テストケース | 前提条件 | テストステップ | 期待結果 |
|----|------------|---------|--------------|---------|
| USER-01 | ユーザー作成 | 適切な権限でログイン済み | 1. ユーザー管理画面にアクセス<br>2. 「新規作成」ボタンをクリック<br>3. ユーザー情報を入力<br>4. 保存ボタンをクリック | ユーザーが作成され、リストに表示される |
| USER-02 | ユーザー編集 | 適切な権限でログイン済み、ユーザーが存在する | 1. ユーザー一覧から編集対象を選択<br>2. 情報を変更<br>3. 保存ボタンをクリック | ユーザー情報が更新される |
| USER-03 | ユーザー削除 | 適切な権限でログイン済み、ユーザーが存在する | 1. ユーザー一覧から削除対象を選択<br>2. 削除ボタンをクリック<br>3. 確認ダイアログで確認 | ユーザーが削除され、リストから消える |
| USER-04 | ユーザー権限検証 | 異なる権限のユーザーが存在する | 1. 異なる権限を持つユーザーでログイン<br>2. 各機能へのアクセスを試みる | 権限に応じたアクセス制御が機能する |
| USER-05 | パスワード変更 | ログイン済み | 1. プロフィール画面にアクセス<br>2. パスワード変更フォームに入力<br>3. 保存ボタンをクリック | パスワードが変更され、新パスワードでログイン可能になる |
