...
- `/gc`: チャット履歴から変更内容が明確な場合のコミット
  - チャット履歴から変更内容を参照し、{コミットの手順}セクションに従って以下を実行：
    1. 変更内容に基づいて適切なコミットメッセージを生成
    2. 生成したメッセージでコミットを実行
  - 例: `/gc`（AIが "feat: プレイヤー移動システムを追加" のようなメッセージを生成）

- `/gcs`: 変更状態を確認してからコミット
  - {コミットの手順}セクションの手順に従って、以下を実行：
...
- `/gs`: 現在の変更状態を確認
  - {コミットの手順}セクションの手順2に従って変更内容を確認
  - 例: `/gs`

- `/gd [file]`: 指定したファイルの差分を確認
  - {Verseファイル差分確認プロセス}に従って差分を確認・解析
  - 例: `/gd game_mode.verse`

これらのコマンドは、チャット欄に直接入力することで実行できます。コマンドを実行すると、AIが自動的に対応するGit操作を行います。
